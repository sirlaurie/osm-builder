...
`work` 遇到连接超时、传输中断或 HTTP 429/5xx 时，在请求重试耗尽后等待 60 秒再尝试，不退出进程。此行为覆盖领取、下载、上传与发布确认；活动任务停止后保留索引和构建产物，退回待处理，再领取租约。未确认发布的数据不执行 `--cleanup`。续租的网络失败在原租约期限内重试，失败不延长期限；过期后必须重新领取。认证失败、无效响应、数据损坏和磁盘错误仍会终止任务。`jobs`、`published-state` 和批次提交等单次操作保持有限重试。
